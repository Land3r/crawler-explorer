...
console.log(data)
  ...
  context.font = `${weight} ${subLabelSize}px ${font}`;
  const clusterLabelWidth = typeLabel ? context.measureText(typeLabel).width : 0;
  const sourceLabelWidth = sourceLabel ? context.measureText(sourceLabel).width : 0;

  const textWidth = Math.max(labelWidth, subLabelWidth, clusterLabelWidth, sourceLabelWidth);
  ...
  const hSourceLabel = sourceLabel ? Math.round(subLabelSize / 2 + 9) : 0;
  const hClusterLabel = Math.round(subLabelSize / 2 + 9);

  drawRoundRect(context, x, y - hSubLabel - 12, w, hClusterLabel + hLabel + hSubLabel + hSourceLabel + 12, 5);
  context.closePath();
  context.fill();
  ...
  context.font = `${weight} ${subLabelSize}px ${font}`;
  context.fillText(typeLabel, data.x + data.size + 3, data.y + size / 3 + 3 + subLabelSize);
}
